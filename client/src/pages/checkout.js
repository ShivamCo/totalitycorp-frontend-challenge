...
export const Checkout = () => {
    ...
    const removeClickHandler = async (event) => {
        ...
        if (!localStorage.getItem("userID")) {
            ...
            try {
                await axios.post(`http://localhost:5000/removeRouter/${localStorage.userID}/${event.target.value}`)
            } catch (error) {
                console.log(error)
            }
        }
    ...
    useEffect(()=> {
        if (!localStorage.getItem("userID")) {
            ...
            try {
                axios.get(`http://localhost:5000/userDetail/${localStorage.userID}`).then((response) => {
